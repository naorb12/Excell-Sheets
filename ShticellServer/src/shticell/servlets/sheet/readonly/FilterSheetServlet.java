...
public class FilterSheetServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // Read and parse the JSON body to get selectedWordsSet
        ...
        Set<String> selectedWordsSet = gson.fromJson(jsonBody.toString(), new TypeToken<Set<String>>() {}.getType());
        ...
        // Perform filtering on the specified range with the selected words
        try {
            SheetDTO filteredSheetDTO = sheetManager.filterSheet(fromCell, toCell, selectedWordsSet);
            if (filteredSheetDTO != null) {
                response.setStatus(HttpServletResponse.SC_OK);
                response.getWriter().write(gson.toJson(filteredSheetDTO));
            } else {
                response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                response.getWriter().write(gson.toJson("Failed to filter sheet."));
            }
        } catch (Exception e) {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            response.getWriter().write(gson.toJson("Error during filtering: " + e.getMessage()));
        }
    }
}